...
const translations: Record<Language, Record<string, any>> = {
  en: {
    ...
    settings: { language: "Language", theme: "Theme", dark: "Dark", light: "Light", back: "Back", sync: "Sync to Cloud", syncDesc: "Automatically back up and sync your data across devices. (This feature is coming soon!)", syncBtn: "Coming Soon", aboutTitle: "About Kairo" },
  ...
  id: {
    ...
    settings: { language: "Bahasa", theme: "Tema", dark: "Gelap", light: "Terang", back: "Kembali", sync: "Sinkronisasi Cloud", syncDesc: "Cadangkan dan sinkronkan data Anda secara otomatis di seluruh perangkat. (Fitur ini akan segera hadir!)", syncBtn: "Segera Hadir", aboutTitle: "Tentang Kairo" },
  ...
  tlh: {
    ...
    settings: { language: "Hol", theme: "Tema'", dark: "qIj", light: "leS", back: "jImev", sync: "CloudDaq Sync", syncDesc: "yIqIp 'ej sync yIghoS. (chu'wI' ghaH.)", syncBtn: "chu'wI'", aboutTitle: "Kairo paq" },
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // This effect auto-prunes any task groups that are no longer associated with any tasks.
    const usedGroupIds = new Set(tasks.map(t => t.taskGroupId).filter(Boolean));
    setTaskGroups(prevGroups => {
        const groupsToKeep = prevGroups.filter(g => usedGroupIds.has(g.id));
        // Only update state if there's a change to prevent infinite loops
        if (groupsToKeep.length === prevGroups.length) {
            return prevGroups;
        }
        return groupsToKeep;
    });
