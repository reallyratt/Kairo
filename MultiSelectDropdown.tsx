...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({ options, selectedValues, onChange, placeholder = 'Select...', className }) => {
  ...
  return (
    <div className={`relative ${className}`} ref={selectRef}>
      ...
      {isOpen && (
        <div className={`absolute z-10 mt-1 w-full bg-slate-700 rounded-lg shadow-lg max-h-60 overflow-auto ${animationClass}`}>
          <ul>
            {options.map(option => (
              <li
                key={option.value}
                className="px-4 py-2 cursor-pointer hover:bg-fuchsia-400/20 flex items-center gap-3"
                onClick={() => handleOptionClick(option.value)}
              >
                <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${selectedValues.includes(option.value) ? 'bg-fuchsia-500 border-fuchsia-500' : 'border-slate-400'}`}>
                    {selectedValues.includes(option.value) && <i className="fa-solid fa-check text-xs text-slate-50"></i>}
                </div>
