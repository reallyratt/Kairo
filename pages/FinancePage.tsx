            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-6 gap-2">
                        {WALLET_ICONS.map(ic => (
                            <button type="button" key={ic} onClick={() => setIcon(ic)} className={`text-lg p-2 rounded-lg transition-colors ${icon === ic ? 'bg-fuchsia-500/30' : 'bg-slate-800'}`} style={{backgroundColor: icon === ic ? 'var(--accent-secondary)' : 'var(--bg-tertiary)', color: icon === ic ? 'var(--accent-text)' : 'var(--text-primary)'}}><i className={`fa-solid ${ic}`}></i></button>
                        ))}
                    </div>
                </div>
                <div>
    ...
    return (
        <div>
            ...
            <div className="p-4 space-y-4">
                ...
                {/* Summary */}
                <div className="grid grid-cols-3 gap-2 text-center bg-slate-900 p-3 rounded-xl" style={{backgroundColor: 'var(--bg-secondary)'}}>
                    <div>
                        <p className="text-xs" style={{color: 'var(--text-secondary)'}}>Income</p>
                        <p className="font-bold text-green-400" style={{color: 'var(--success-primary)'}}>{formatCurrency(summary.income)}</p>
                    </div>
                    <div>
                        <p className="text-xs" style={{color: 'var(--text-secondary)'}}>Expenses</p>
                        <p className="font-bold text-red-400" style={{color: 'var(--danger-primary)'}}>{formatCurrency(summary.expense)}</p>
                    </div>
                     <div>
                ...
                <div className="space-y-2">
                    <div className="bg-slate-900 p-2 rounded-xl" style={{backgroundColor: 'var(--bg-secondary)'}}>
                        ...
                        return (
                            <button key={t.id} onClick={() => { setEditingTransaction(t); setIsFormOpen(true); }} className="w-full text-left flex gap-3 p-3 rounded-lg" style={{backgroundColor: 'var(--bg-secondary)'}}>
                                ...
                                <div className="flex-shrink-0 text-right">
                                    <p className={`font-bold ${isIncome ? 'text-green-400' : 'text-red-400'}`} style={{color: isIncome ? 'var(--success-primary)' : 'var(--danger-primary)'}}>
